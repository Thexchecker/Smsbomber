...
while 1:
    ...
    if len(content.split(" ")) == 2 and content.split(" ")[0] == ".sms":
                ...
                for attribute in dir(SendSms):
                    ...
                    if callable(attribute_value):
                        if attribute.startswith('__') == False:
                            ...
                            exec("sms."+attribute+"()")
                            sleep(saniye)
            send(token, chat_id, telno+" --> "+str(sms.adet)+f" adet SMS gönderildi. The X\n<@{id}>")                        
        else:
        ...
        zaman.append(timestamp)
        send(token, chat_id, f"Sms göndermek için komutu aşağıdaki gibi yazınız. (TheX) \n.sms 5051234567\n.sms (telefon numarası)\n<@{id}>")
    elif ".status" == content and timestamp not in zaman:
        zaman.append(timestamp)
        mesaj = "TheX" #mesajınızı yazınız
